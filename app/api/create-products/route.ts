...
export async function POST(req: Request) {
  ...
  try {
    ...
    const addProducts = await Promise.all(sanityProducts.map(async (sanityProduct: any) => {
    ...
    return NextResponse.json({ success: true, data: addProducts }), 
    {
      status: 200,
    ...
    console.error(err);
    return NextResponse.json({error: `Error creating - ${err}  `,  status: 500 });
